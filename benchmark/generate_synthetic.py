...
def build_lab_report(
    patient_id: str, report_date: str, ordering_provider: str
) -> LabReport:
    return LabReport(
...
def render_pdf(path: Path, report: LabReport) -> None:
    ...
    story.append(
        Paragraph(f"Provider: {report.ordering_provider or ''}", styles["Normal"])
    )
...
def generate_dataset(
    *, output_dir: str | None = None, count: int = 5
    ...
    for i in range(count):
        ...
        report = build_lab_report(
            patient_id=pid, report_date=report_date, ordering_provider="Dr. Bot"
        )
